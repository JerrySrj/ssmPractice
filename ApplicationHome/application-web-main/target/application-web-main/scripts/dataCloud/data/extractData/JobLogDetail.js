...
function getJogLogField() {
	var param = {
			"jobName": $("#jobName").val(), 
			"frontMachineKey": $("#frontMachineKey").val(), 
			"batchId" : $("#batchId").val(),
	};
	$.post(ctx + "/extractDataJob/getJobLogField", param, function(data, status) {
		if("success" == status) {
			var logField = "<textarea id='logContent' class='form-control' style='resize:none;margin-top:10px;' readonly rows='10' cols=''>"
						+ data + "</textarea>";
			$("#originalLogDiv").empty();
			$("#originalLogDiv").append(logField);
		}else
			showErrorMsg("加载原始日志发生异常，请联系系统管理员！");
	});
		...
		if(status == "success") {
			...
			if(structuredLogList.length > 0) {
				for(var i = 0; i < structuredLogList.length; i++) {
					...
					dataTr += "<td style='text-align:center;'><button class='btn btn-xs btn-info' onclick='showErrorLogPop(\"" + structuredLogList[i].guid + "\")'>显示日志</button></td>";
					dataTr += "</tr>";
				}
			} else {
...
function searchLog(flag) {
	...
	if("all" == flag) {
		...
		$("#structuredDateFilter").removeClass("active");
...
function structuredChangePage(flag) {
	...
	if(!$("#structuredDateFilter").hasClass("active")) {
		param.beginDate =  new Date($("#beginDate").val() + " 00:00:00").format("yyyy-MM-dd hh:mm:ss");
		param.endDate = new Date($("#endDate").val() + " 23:59:59").format("yyyy-MM-dd hh:mm:ss");
	}
	switch (flag) {
	...
	case 'last':
		...
		break;
	default:
		param.currentPage = $("#structuredGoPage").val() > parseInt($("#totalPage").text()) ? $("#totalPage").text() : $("#structuredGoPage").val();
		break;
	}
	getStructuredLog(param);
}
